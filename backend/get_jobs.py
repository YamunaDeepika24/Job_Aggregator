...
@router.get("/matching", response_model=List[schemas.JobOut])
def get_matching_jobs(
    ...
    if prefs.role:
        query = query.filter(models.Job.title.ilike(f"%{prefs.role}%"))

    if prefs.location:
        query = query.filter(models.Job.location.ilike(f"%{prefs.location}%"))

    if prefs.experience:
        query = query.filter(models.Job.experience <= prefs.experience)
...
# -------- NORMALIZE JOBRIGHT RESPONSE --------
def normalize_jobright(data):
    jobs = []
    if not data:
        return jobs
    ...
    try:
        ...
        for item in joblist:
            jr = item.get("jobResult", item)

            jobs.append({
                "title": jr.get("jobTitle") or "",
                "company": jr.get("companyName") or "",
                "location": jr.get("jobLocation") or "",
                "posted": jr.get("publishTimeDesc") or "",
